...
def get_data_from_file(file, red_wl, ir_wl):
    with open(file) as f:
    ...
    if(red_wl is not None or ir_wl is not None):
        for i in raw_data[reflect_index:len(raw_data)]:  # Get reflect data from file
            var = i.split("\t")
            wavelength= float(var[0])
            if (wavelength < ir_wl) and (wavelength > red_wl):
...
def set_up_data(red_wl = None, ir_wl = None):
    reflect_data = []
    ...
    for filename in os.listdir(directory):
        ...
        # print(red_wl, ir_wl)
        a = get_data_from_file(file_dir, red_wl, ir_wl)
        if a is not None:
