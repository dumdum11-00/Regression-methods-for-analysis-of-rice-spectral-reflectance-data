...
def linear_regression(red_wl, ir_wl):   
    log_data= [datetime.now().strftime("%d/%m/%Y, %H:%M:%S")]
    log_data.append("Linear Regression")
    log_data.append("")
    log_data.append(red_wl)
    log_data.append(ir_wl)    
    ...
    # grid_linear_performance.fit(X_train_scaled, y_train)
    ...
    #             }
    # grid_linear_performance = GridSearchCV(LinearRegression(), param_grid = random_grid, scoring = 'r2', cv=5)
    # grid_linear_performance.fit(X_train_scaled, y1_train)
    # print(grid_linear_performance.best_params_)
    ...
    #             }
    # grid_linear_performance = GridSearchCV(LinearRegression(), param_grid = random_grid, scoring = 'r2', cv=5)
    # grid_linear_performance.fit(X_train_scaled, y2_train)
    # print(grid_linear_performance.best_params_)
    ...
    with open('result_nvdi.csv', 'a') as file:
